...
class ArticleController extends Controller
{
    ...
    public function store(ArticleRequest $request)
    {
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $name = \StringHelper::makeSlug($coverImage->getClientOriginalName());
            $articleImages = Carbon::now()->timestamp . '-' .$name.".".$ext;
            $coverImage->move(public_path('uploads/article/'), $articleImages);
                $articlePath = '/uploads/article/' . $articleImages;
                $this->articleRepository->update($model, [
                    'images_url' => $articlePath
                ]);
